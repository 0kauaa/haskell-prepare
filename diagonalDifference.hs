...
--
getDiagonal :: [[Int]] -> [Int]
getDiagonal m = Data.List.zipWith (!!) m [0..]

diagonalDifference :: [[Int]] -> Int
diagonalDifference arr = do
    let ldiag = getDiagonal arr
        rdiag = getDiagonal (Data.List.map Data.List.reverse arr)
        
        lsum = Data.List.foldl (+) 0 ldiag 
        rsum = Data.List.foldl (+) 0 rdiag
    abs (lsum - rsum)
